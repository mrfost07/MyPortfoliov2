...
import { Trash2, Edit2, ChevronUp, ChevronDown, Award } from 'lucide-react';
...
export default function CertificatesForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                order_index: editingId ? formData.order_index : certificates.length
            };
            ...
            if (editingId) {
                ...
                const { error } = await supabase.from('certificates').insert(insertPayload);
                if (error) throw error;
    ...
    const resetForm = () => {
        setFormData({ title: '', issuer: '', date: '', description: '', image: '' });
        setEditingId(null);
    };
    ...
    const moveItem = async (index, direction) => {
        ...
        try {
            for (const update of updates) {
                await supabase.from('certificates').update({ order_index: update.order_index }).eq('id', update.id);
            }
            fetchCertificates();
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
            <h2 className="text-xl font-bold flex items-center gap-2">
                <Award className="text-[#16f2b3]" size={24} />
            ...
            <form onSubmit={handleSubmit} className="space-y-4 bg-[#0d1224] p-4 rounded-lg border border-[#2a3241]">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex flex-col">
                        <label className="mb-1 text-sm text-gray-400">Certificate Title *</label>
                        <input
                            name="title"
                    ...
                    <div className="flex flex-col">
                        <label className="mb-1 text-sm text-gray-400">Issuing Organization</label>
                        <input
                            name="issuer"
                    ...
                    <div className="flex flex-col">
                        <label className="mb-1 text-sm text-gray-400">Date Obtained</label>
                        <input
                            name="date"
                            value={formData.date}
                            onChange={handleChange}
                    ...
                    <div className="flex flex-col md:col-span-2">
                        <label className="mb-2 text-sm text-gray-400">Certificate Image</label>
                        <input
                            type="file"
                            accept="image/*"
                        ...
                        {formData.image && (
                            <div className="mt-3">
                                <img src={formData.image} alt="Preview" className="max-h-48 w-auto object-contain rounded-lg border border-[#2a3241]" />
                            </div>
                        )}
                    </div>
                </div>
                <div className="flex flex-wrap gap-3 pt-2">
                    ...
                    >
                        {uploading ? 'Uploading...' : editingId ? 'Update' : 'Add Certificate'}
                    </button>
                    {editingId && (
                        <button
                            type="button"
            ...
            <div className="space-y-3">
                <h3 className="text-lg font-semibold text-gray-300">Your Certificates ({certificates.length})</h3>
                {certificates.length === 0 ? (
                    <p className="text-gray-500 text-center py-8">No certificates yet. Add your first one above!</p>
                ) : (
                    certificates.map((item, index) => (
                        <div key={item.id} className="bg-[#0d1224] p-4 rounded-lg border border-[#2a3241] flex flex-col sm:flex-row gap-4">
                            ...
                            <div className="flex-1 flex flex-col sm:flex-row gap-4">
                                {item.image && (
                                    <img src={item.image} alt={item.title} className="w-full sm:w-32 h-24 object-cover rounded-lg border border-[#2a3241]" />
                                )}
                                <div className="flex-1">
                                    <h4 className="font-bold text-lg">{item.title}</h4>
                                    {item.issuer && <p className="text-sm text-[#16f2b3]">{item.issuer}</p>}
